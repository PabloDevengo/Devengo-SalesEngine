    ...
    <div className="px-8 py-6 max-w-3xl w-full space-y-5">
          ...
          <div className="px-6 py-4 border-t border-gray-100 space-y-3">
            ...
            <button onClick={enviar} disabled={loading || !webhookUrl.trim()}
              ...
              {loading
                ...
                : <><Zap size={13} />{webhookUrl.trim() ? `Generar ${numEmails} email${numEmails > 1 ? "s" : ""}` : "Pega la URL del webhook para enviar"}</>}
            </button>
          </div>
        </section>
      )}
      ...
      {!canSend && !loading && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-gray-400">Selecciona un producto, añade personas y un dominio para empezar.</p>
        </div>
      )}
    </div>
  );
